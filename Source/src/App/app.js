...
function renderGroups() {
  ...
  for (const group of groups) {
    ...
    item.querySelector(".group-select").addEventListener("click", () => {
      ...
      state.avatarPage = 0;
      if (state.activePage !== "favorites") state.activePage = "favorites";
      render();
    });
...
function renderAvatars() {
  ...
  for (const avatar of visibleAvatars) {
    ...
    const startAvatarDrag = (event) => {
      ...
      state.dragSort = { type: "avatar", id: avatar.id, groupId: avatar.groupId, dragWidth: rect.width, dragHeight: rect.height, blockedSynced: syncedReorderBlocked, copyOnly: canDragAvatarsToGroup };
...
function showAvatarContextMenu(event, avatar, canReorderCurrentGroup) {
  ...
  const actions = [
    ...(canPlacePending ? [
      ...
      { label: "Sort Avatar", className: "separated", disabled: !canReorderCurrentGroup, action: sortAction }
    ] : [
      { label: "Sort Avatar", disabled: !canReorderCurrentGroup, action: sortAction }
    ]),
    { label: "Move to Group", disabled: systemSourceGroup, action: () => openAvatarGroupActionDialog(avatar, "move") },
...
function databasePlatformBadgeLabels(value) {
  ...
  for (const item of splitBadgeValues(value)) {
    ...
    const label = lower.includes("standalonewindows") || /\b(windows|pc)\b/.test(lower) ? "PC" : lower.includes("android") || lower.includes("quest") ? "Quest" : lower.includes("ios") ? "iOS" : "";
    if (label && !labels.some((x) => x.label.toLowerCase() === label.toLowerCase())) labels.push({ label, className: label.toLowerCase() === "pc" ? "platform-pc" : label.toLowerCase() === "quest" ? "platform-quest" : "platform-ios" });
  }
  return labels;
}
...
async function runAvatarDatabaseSearch(page = 0) {
  ...
  try {
    ...
    if (page === 0 && state.avatarDatabaseResults.length && state.avatarDatabaseTotal == null && !["all", "vrcx"].includes(avatarDatabaseProvider())) countAvatarDatabaseTotal(query, token, databaseSearchPayload(query, 0));
...
function hideContextMenu() { $("contextMenu").hidden = true; hideSortMenu(); hideSortMenu("databaseSortMenu", "databaseSortMenuBtn"); hideSortMenu("groupFilterMenu", "groupFilterMenuBtn"); hideSortMenu("avatarDatabaseProviderMenu", "avatarDatabaseProviderMenuBtn"); }
...
function showDeletedScanResult(result) {
...
async function loadSettingsLogs() {
  ...
  try {
    ...
    list.innerHTML = entries.length
      ...
      : `<div class="settings-empty"><h4>No logs yet</h4><p>Useful app events, sync results, backups, updates, and errors will appear here.</p></div>`;
  } catch (e) {
